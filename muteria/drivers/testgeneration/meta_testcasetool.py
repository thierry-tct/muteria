...
class MetaTestcaseTool(object):
    ...
    def __init__(self, tests_working_dir,
        ...
        assert len(tests_toolname_list) == len(config_list), \
        ...
        for idx in range(len(tests_toolname_list)):
            ...
            self.testcases_tools[toolname] = {
                "tool_obj": self.get_testcase_tool(language, toolname, tool_working_dir, config, tool_checkpointer),
                "tool_working_dir": tool_working_dir,
            }
    #~ def __init__()

    def get_testcase_tool(self, language, toolname, tool_working_dir, config, tool_checkpointer):
        '''
            Each tool module must have the function createTestcaseTool() implemented
        '''
        testcase_tool = self.modules_dict[language][toolname].createTestcaseTool(
                                            tool_working_dir,
                                            self.code_builder,
                                            config,
                                            tool_checkpointer)
    ...
    def execute_testcase (self, meta_testcase, exe_path, env_vars):
        ...
        assert ttoolname in testcases_tools, "tool not registered: "+ttoolname
        return self.testcases_tools[ttoolname]['tool_obj'].execute_testcase(testcase,
                                                                            exe_path, 
                                                                            env_vars)
    #~ def execute_testcase()

    def runtests(self, meta_testcases, exe_path, env_vars, stop_on_failure=False, 
                                            fault_test_execution_matrix=None):
        '''
        ...
        # Get the saved payload (data kapt for each tool)
        ...
        for tpos, ttoolname in enumerate(testcases_by_tool.keys()):
            ...
            if not checkpoint_handler.is_to_execute(func_name="runtests", \
                                                taskid=1, \
                                                tool=ttoolname):
                continue

            # Actual execution
            test_failed_verdicts = self.testcases_tools[ttoolname]['tool_obj'] \
                                    .runtests(testcases_by_tool[ttoolname], \
                                                exe_path, env_vars, \
                                                stop_on_failure)
            ...
            # Chekpointing
            checkpoint_handler.do_checkpoint(func_name="runtests", \
                                        taskid=1, \
                                        tool=ttoolname, \
                                        opt_payload=meta_test_failed_verdicts)

            if stop_on_failure and found_a_failure:
                # Chekpointing for remaining tools
    ...
    def generate_tests (self):
        ...
        for ttoolname in self.testcases_tools:
            ...
            if checkpoint_handler.is_to_execute( \
                ...
                # Actual Execution
                self.testcases_tools[ttoolname]['tool_obj'].generate_tests()
        ...
        for ttoolname in self.testcases_tools:
            tool_testcase_info = self.testcases_tools[ttoolname]['tool_obj'].\
                                                        get_testcase_info_object()
